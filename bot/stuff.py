...
async def start(event):
    await event.reply(
        ...
        buttons=[
            ...
            [Button.url('Update', url='t.me/supernovanetwork')],
            [Button.url('Support', url='t.me/paradox_chatz_group')]
        ],
    )
...
async def help(event):
    await event.edit(
        f"""**To check current ffmpeg code you can use** /getcode\n\n**You can change your ffmpeg code by executing following command.**\n\n`/setcode -preset faster -c:v libx265 -s 1280x720 -x265-params 'bframes=8:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1' -metadata 'title=Encoded By @Not_Xenov (https://t.me/xenov7x)' -pix_fmt yuv420p -crf 30 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1`\n\n**To set custom thumbnail send me the image.**\n\n**Do /cmds For More**"""
    )
